...
int parse_json(const char* json_path, api_metadata* api_md){
	...
	for(int i=0;i<KEY_COUNT-1;i++){ // -1 cuz we need to handle last key manually since it as [] in value field.
		...
		if(i==0){
			...
			printf("m0 : %ld | %c %c %c\n",marker0,*marker0,*(marker0+1),*(marker0+2));
		...
		printf("m1 : %ld | %c %c %c\n",marker1,*marker1,*(marker1+1),*(marker1+2));
		memcpy(api_md,marker0,marker1-JSON_STRUCTURAL_DIFF-marker0);
		printf("copied str : %s\n",(char*)api_md);
		
		printf("uppdating marker0...\n");
		printf("size marker1 substr : %ld\n",strlen(json_key_array[i+1]));
		marker0=marker1+strlen(json_key_array[i+1])+JSON_STRUCTURAL_DIFF;
		printf("m0 now : %ld | %c %c %c %c %c %c\n",marker0,*marker0,*(marker0+1),*(marker0+2),*(marker0+3),*(marker0+4),*(marker0+5));
